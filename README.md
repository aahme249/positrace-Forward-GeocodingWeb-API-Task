...
- **No input plausibility validation** — a garbage/non-address string is not rejected before being sent to Nominatim. Nominatim's free-text search returns its best loose guess rather than "no match" for a degenerate query, so e.g. the input `"12"` comes back `strategy: "address", found: true` matched against an unrelated electoral district, not a real street address. `found: true` currently only means "Nominatim returned something," not "this is a genuine address match." **To be fixed**: reject or short-circuit inputs that can't plausibly be a street address (at minimum, require at least one letter) before they reach Nominatim.
