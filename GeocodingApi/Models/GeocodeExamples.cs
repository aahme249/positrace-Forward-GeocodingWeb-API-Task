...
public class GeocodeExampleFilter : IOperationFilter
{
    public void Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        resMedia.Example = new OpenApiObject
        {
            ...
            {
                // 1. dash-unit strip (unit 123, civic 12 per Canada Post convention) → not_found
                //    (fake street number); postal fallback was attempted, so retryCount is 1
                Result("123-12 Main St, Toronto, ON M5V 2T6",
                       "12 Main St, Toronto, ON M5V 2T6",
                       strategy: "not_found", retryCount: 1),
                ...
                // 4. Suite qualifier → postal_code fallback (retryCount 1: the fallback call itself)
                ...
                // 8. fake street, no postal code extractable → not_found; only the address search
                //    was attempted (no fallback possible), so retryCount is 0
                Result("Unit 5 99999 Nowhere Blvd, Faketown, ON",
                       "99999 Nowhere Blvd, Faketown, ON",
                       strategy: "not_found", retryCount: 0),
                ...
                // 13. duplicate #1 → address (Nominatim called once, retryCount 0)
                ...
                // 14. duplicate #2 → address, served from dedup/cache — no Nominatim call was
                //     made for this one, so retryCount is null (not 0)
                Result("100 Queen St W, Toronto, ON M5H 2N2",
                       "100 Queen St W, Toronto, ON M5H 2N2",
                       43.6536032, -79.3840055,
                       "Toronto City Hall, 100, Queen Street West, Toronto, Ontario, Canada",
                       "address", retryCount: null),
            }
        };
    }
    ...
    private static OpenApiObject Result(
        ...
        new()
        {
            ...
            ["retryCount"]        = retryCount.HasValue ? new OpenApiInteger(retryCount.Value) : new OpenApiNull(),
        };
}
